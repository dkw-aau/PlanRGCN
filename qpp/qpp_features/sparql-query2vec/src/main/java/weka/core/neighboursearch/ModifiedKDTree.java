...
public class ModifiedKDTree
     ...
     * should be built on.
     */
    public ModifiedKDTree(Instances insts) {
        super(insts);
        if (getMeasurePerformance())
            m_Stats = m_TreeStats = new TreePerformanceStats();
    }
     ...
     * @throws Exception 	if the nearest neighbour could not be found.
     */
    public int[] kNearestNeighboursModified(Instance target, int k) throws Exception {
        checkMissing(target);
